<template>
  <div class="dashboard-view">
    ...
    <el-row :gutter="20" class="stats-row">
      <el-col v-for="stat in statistics" :key="stat.key" :xs="12" :sm="8" :md="4">
        <StatCard
          :icon="stat.icon"
          :label="stat.label"
          :value="stat.value"
    ...
    <el-row :gutter="20" class="charts-row">
      ...
      <el-col :xs="24" :md="14">
        <el-card class="chart-card">
          ...
          <ResourcePieChart v-if="statisticsData" :data="statisticsData" />
        </el-card>
      </el-col>
      ...
      <el-col :xs="24" :md="10">
        <el-card class="chart-card">
          ...
          <ActivityTimeline v-if="activities" :activities="activities" />
        </el-card>
      </el-col>
    </el-row>
...
const loadStatistics = async () => {
  try {
    ...
    statistics.value[0].subtitle = `活跃 ${data.knowledgeBases.activeCount} / 归档 ${data.knowledgeBases.archivedCount}`
    statistics.value[0].trend = data.knowledgeBases.weeklyGrowthRate

    statistics.value[1].value = data.agents.totalCount
    statistics.value[1].subtitle = `ReAct ${data.agents.reactCount} / Workflow ${data.agents.workflowCount}`
    statistics.value[1].trend = data.agents.monthlyNewCount

    statistics.value[2].value = data.chatbots.totalCount
    statistics.value[2].subtitle = `已发布 ${data.chatbots.publishedCount} / 草稿 ${data.chatbots.draftCount}`

    statistics.value[3].value = data.documents.totalCount
    statistics.value[3].subtitle = `处理中 ${data.documents.processingCount} / 已完成 ${data.documents.completedCount}`

    statistics.value[4].value = data.users.totalCount
    statistics.value[4].subtitle = `管理员 ${data.users.adminCount} / 普通用户 ${data.users.regularCount}`

    statistics.value[5].value = Number(
      (data.storage.totalSizeBytes / (1024 * 1024 * 1024)).toFixed(1)
    )
    statistics.value[5].subtitle = `文件 ${data.storage.fileCount} 个`
  } catch (error) {
